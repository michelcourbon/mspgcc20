...
""")
        ...
        group.add_option("--setSerial",
                dest="serial_ID",
                help="Set the device serial ID",
                default=[],
                action='append',
                metavar="SERIALID")
...
""")
    ...
    def parse_args(self):
        ...
        for a in self.options.serial_ID:
            try:
                self.add_action(self.set_serial_nb, a)
            except ValueError, e:
                self.parser.error("--Serial_ID: %s" % e)
