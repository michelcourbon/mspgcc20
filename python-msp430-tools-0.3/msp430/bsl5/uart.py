...
class SerialBSL5(bsl5.BSL5):
    ...
    def bsl(self, cmd, message='', expect=None):
        ...
        calculated_crc = crc_update(0xffff, chr(cmd))
        for s in range(0,(len(message))):
            if(isinstance(message[s], int)):
                calculated_crc = crc_update(calculated_crc, chr(message[s]))
            else:
                calculated_crc = crc_update(calculated_crc, message[s])
            
        txdata = struct.pack('<BHB', 0x80, 1+len(message), cmd) + message      
        txdata += struct.pack('<H', calculated_crc)   # append checksum
        
        #~ self.logger.debug('Sending command: %r' % (txdata.encode('hex'),))
        # transmit command        
        self.serial.write(txdata)
        #print ('Sending command: %r' % (txdata.encode('hex'),))
        ...
        if chr(pi) == '\x80':
            ...
            for s in range(0,(len(data))):
               crc_expected = crc_update(crc_expected, data[s])
            
            if (crc[0] != crc_expected):
                raise bsl5.BSLException('CRC error in answer')
            
            if expect != 0 and length != expect:
                raise bsl5.BSL5Error('expected %d bytes, got %d bytes' % (expect, len(data)))
            
            return data
        else:
            if pi: raise bsl5.BSL5Error('received bad PI, expected 0x80 (got 0x%02x)' % (ord(chr(pi)),))
